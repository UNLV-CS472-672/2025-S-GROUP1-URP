/**
 * File: CottageGroveParkingScreen.js
 * Purpose: Provides a screen for users to view and reserve parking spots in the Cottage Grove Parking lot.
 * Dependencies:
 *   - React and React Native components for UI rendering.
 *   - Firebase Firestore for managing parking spot data.
 *   - Firebase Auth for user authentication.
 *   - React Native Gesture Handler and Reanimated for gesture-based map interactions.
 *   - ImageBackground for displaying the parking lot map.
 * Usage:
...
const ParkingMap = ({ parkingLot = "Tropicana Parking" }) => {
  ...
  const scale = useSharedValue(1); // Scale for pinch-to-zoom gesture
  const startScale = useSharedValue(0); // Initial scale value for pinch gesture

  const translationX = useSharedValue(0); // X-axis translation for pan gesture
  const translationY = useSharedValue(0); // Y-axis translation for pan gesture
  const prevTranslationX = useSharedValue(0); // Previous X-axis translation
  const prevTranslationY = useSharedValue(0); // Previous Y-axis translation

  /**
   * Gesture for pinch-to-zoom functionality.
   */
  const pinch = Gesture.Pinch()
    ...
    .onUpdate((event) => {
      scale.value = clamp(startScale.value * event.scale, 1, 3);
    })
    .runOnJS(true);

  /**
   * Gesture for panning the map.
   */
